...
private _trainPositionASL = getPosASLVisual _trainCar;
private _trainVectorDir = vectorDirVisual _trainCar;
private _offset = _trainCar getVariable ["ATRAIN_Remote_Position_Offset",[0,0,0]];
_trainPositionASL = _trainPositionASL vectorAdd (_offset vectorMultiply -1);
private _tracks = [_trainPositionASL,_trainCar,_trainVectorDir] call ATRAIN_fnc_getTracksAtPosition;
...
if(count _tracks == 0) then {
	...
	for "_i" from 0 to 25 do
	{
		...
		_tracks = [_centerPoint, _trainCar] call ATRAIN_fnc_getTracksAtPosition;
