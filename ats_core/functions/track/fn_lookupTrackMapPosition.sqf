...
private _track = [_trainCar] call ATRAIN_fnc_getTrackUnderTrainCar;
...
scopeName "ATRAIN_fnc_lookupTrackMapPosition_0";
{
	if(_x select 0 == _track) then {
		if( vectorDirVisual _trainCar vectorDotProduct (_x select 1) > 0 ) then {
...
if(count _foundTrackLookup > 0) then {
	...
	private _trianPositionASL = getPosASLVisual _trainCar;
